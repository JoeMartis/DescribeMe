...
Worth knowing what "won't update" looks like in practice, because it is quiet:
on an existing clone the badge does not read "dev", it keeps showing whatever
number was last committed while the app moves on without it. If the badge and
the commit count disagree, the hook is not enabled.

## Tests

`test/` holds browser regression suites — headless Chromium driving the real
app over a real HTTP origin, since ES modules, IndexedDB, downloads and the
CSP all behave differently over `file://`.
...
`run.mjs` starts its own static server on a port the OS picks, and exits
non-zero if anything fails. No API key is needed: every suite that exercises a
describe run intercepts the API call and answers it locally, and several assert
on the *number* of intercepted calls, so a suite that reached the network would
fail rather than quietly bill someone. `test/README.md` says what each suite
covers.
...
- `test/` — browser regression suites and the runner that serves the app to
  them; the only place in the repo npm is involved
