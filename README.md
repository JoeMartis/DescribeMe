...
- A `Content-Security-Policy` meta tag adds a second, independent layer on
  ...
  'self'` — the font is self-hosted from `fonts/`, see Files below), and
  `connect-src` restricted to the MIT Parley endpoint — so even a
...
- `style.css` — styling: the Modernist design system (flat, architectural,
  set entirely in Archivo — light ground, one red accent, zero corner
  radius, structural rules instead of cards/shadows); a single light
  palette, no dark-mode scope
...
- `fonts/` — self-hosted `Archivo-Variable.woff2` (see the note at the top
  of `style.css`); without it the fallback font stack is used and nothing
  breaks
